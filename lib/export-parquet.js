...
async function saveToParquet(columnDetails, rows, tableNameOnly) {
    ...
    console.log(`Parquet schema: ${JSON.stringify(schemaDefinition)}`)
    ...
    await writer.close()
    console.log(`Wrote ${rows.length} rows to ${localFilePath}`)

    const s3Key = `${s3Prefix}/${tableNameOnly}.parquet`
    await uploadS3(fs.createReadStream(localFilePath), s3Key)
    console.log(`Uploaded ${localFilePath} to S3`)
    ...
    try {
        ...
        for (const table of tablesToExport) {
            try {
                ...
                console.log(`Exporting table ${table} with ${columnDetails.length} columns`)
                const { fields, rows } = await fetchTableData(table, client)
                console.log(`Fetched ${rows.length} rows for table ${table}`)
